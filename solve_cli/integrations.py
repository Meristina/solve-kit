...
.solve/commands/) into the chosen AI agent's directory, reformatted per agent.
...
def install(agent: str, repo_root: Path, target: Path) -> dict:
    ...
    src_commands = repo_root / ".solve" / "commands"
    summary = {"agent": agent}

    if agent == "claude":
        cmds = _copy_commands(src_commands, target / ".claude" / "commands")
        # the engine the commands drive: the Claude mirror
        ...
        summary.update(commands=cmds, agents=len(list((repo_root / "agents").glob("*.md"))),
                       skills=sum(1 for d in (repo_root / "skills").iterdir() if d.is_dir()))
    elif agent == "cursor":
        cmds = _copy_commands(src_commands, target / ".cursor" / "rules", prefix="solve.")
        summary.update(commands=cmds, note="cursor: command bodies copied as rules (refine .mdc as needed)")
    elif agent == "copilot":
        cmds = _copy_commands(src_commands, target / ".github" / "prompts", prefix="solve.")
